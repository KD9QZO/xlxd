...
CDvFramePacket *CXlxProtocol::IsValidDvFramePacket(const CBuffer &Buffer) {
	...
	if ((dvframe == NULL) && (Buffer.size() == 45) && (Buffer.Compare((uint8*)"DSVT", 4) == 0) && (Buffer.data()[4] == 0x20) && (Buffer.data()[8] == 0x20) && ((Buffer.data()[14] & 0x40) == 0)) {
		// create packet
...
int CXlxProtocol::GetConnectingPeerProtocolRevision(const CCallsign &Callsign, const CVersion &Version) {
	...
	if (Callsign.HasSameCallsignWithWildcard(CCallsign("BM*"))) {	/* BrandMeister ? */
