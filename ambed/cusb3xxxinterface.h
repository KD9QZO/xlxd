...
protected:
	// decoder helper
	virtual bool IsValidChannelPacket(const CBuffer&, int*, CAmbePacket*) {
		return (false);
	}

	virtual bool IsValidSpeechPacket(const CBuffer&, int*, CVoicePacket*) {
		return (false);
	}
	...
	virtual int GetDeviceFifoSize(void) const {
		return (1);
	}
